...
var (
	...
	TbcTestnetFlag = cli.BoolFlag{
		Name:  "tbctestnet",
		Usage: "Tbc network: pre-configured Tbc testnet",
	}
...
func MakeDataDir(ctx *cli.Context) string {
	if path := ctx.GlobalString(DataDirFlag.Name); path != "" {
		...
		if ctx.GlobalBool(TbcTestnetFlag.Name) {
			return filepath.Join(path, "tbctestnet")
		}
...
func setBootstrapNodes(ctx *cli.Context, cfg *p2p.Config) {
	...
	case ctx.GlobalBool(TbcTestnetFlag.Name):
		urls = params.TbcTestnetBootnodes
...
func setDataDir(ctx *cli.Context, cfg *node.Config) {
	...
	case ctx.GlobalBool(TbcTestnetFlag.Name):
		cfg.DataDir = filepath.Join(node.DefaultDataDir(), "tbctestnet")
	case ctx.GlobalBool(RinkebyFlag.Name):
...
func SetEthConfig(ctx *cli.Context, stack *node.Node, cfg *eth.Config) {
	...
	checkExclusive(ctx, DeveloperFlag, TestnetFlag, TbcTestnetFlag, RinkebyFlag)
	...
	case ctx.GlobalBool(TbcTestnetFlag.Name):
		...
		cfg.Genesis = core.DefaultTbcTestnetGenesisBlock()
	case ctx.GlobalBool(DeveloperFlag.Name):
		if !ctx.GlobalIsSet(NetworkIdFlag.Name) {
			cfg.NetworkId = 1337
		}
...
func MakeGenesis(ctx *cli.Context) *core.Genesis {
	...
	case ctx.GlobalBool(TbcTestnetFlag.Name):
		genesis = core.DefaultTbcTestnetGenesisBlock()
	case ctx.GlobalBool(RinkebyFlag.Name):
